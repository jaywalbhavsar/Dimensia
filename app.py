...
def send_email_notification(subject, body, recipient):
    ...
    # Run in background thread so login is fast
...
# --- 5. LOGIN FLOWS (WITH EMAIL TRIGGER) ---
...
def google_callback():
    try:
        ...
        send_email_notification(
            "Welcome to Dimensia | Access Granted", 
            f"Hello {user_info['name']},\n\nYou have successfully accessed the Dimensia Network via Google.\n\n— Dimensia Systems", 
            user_info['email']
        )
...
def github_callback():
    try:
        ...
        if email: 
            ...
            send_email_notification(
                "Welcome to Dimensia | Access Granted",
                f"Hello {user_info['login']},\n\nYou have successfully accessed the Dimensia Network via GitHub.\n\n— Dimensia Systems",
                email
            )
...
def handle_contact():
    ...
    send_email_notification("Thank you for contacting Dimensia", f"Dear {name},\n\nWe have received your transmission. Our architects will review your project brief shortly.\n\n— Dimensia Team", email)
