...
if not GOOGLE_CLIENT_ID or not GOOGLE_CLIENT_SECRET:
    print("❗ ERROR: Missing Google OAuth credentials. Check Render environment variables.")
...
def send_email_notification(subject, body, recipient):
    if not recipient: 
        print("⚠️ Email recipient missing.")
        return
...
def google_callback():
    try:
        ...
        if not user_info:
            return "⚠️ Google did not return user info."

        session['user'] = user_info
        send_email_notification("Welcome to Dimensia", f"Hello {user_info.get('name', 'user')}, welcome.", user_info.get('email'))
